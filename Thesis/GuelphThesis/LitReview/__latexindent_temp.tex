...
$lc_{i}$ and match each $1$ in $lc_{i}$ with a $0$ in line $lc_{i+1}$. line $i+1$
will have as many right end points as there are left end points in line $i$. 
...
end point to line $i+1$ relative to where the $1$ occured to their left and 
right neigbor in $lc_{i+1}$.
